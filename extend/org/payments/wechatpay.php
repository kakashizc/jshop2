...
class wechatpay implements Payment
{
    ...
    public function pay($paymentInfo){
        ...
        $discount = DB::name('discount') -> where(['check' => $res]) -> value('discount');//获取身份和折扣
        $discount = 1;//
    ...
    public function callback(){
        ...
        if($data && $data['return_code'] == 'SUCCESS' && $data['sign'] == $this->makeSign($data)){
            ...
            if($data['result_code'] == "SUCCESS"){
                ...
                //获取订单里面所有购买的商品返佣信息,然后返佣金.
                $goods = Db::name('order_items')->where('order_id',$orderInfo['order_id'])->select()->toArray();

                //做用户赏金不同操作问题
                $addMoney = $this -> allMoney($orderInfo,$goods);
                //判断数据是不是唯一的
                if( $addMoney != 10014  && $orderInfo['pid'] != 0  ){
                    file_put_contents('tttt2.txt', $addMoney['mixpeople']);
                    //判断总人数 +1 < 最大上限人数
                    if($addMoney['allCount']+1 <= $addMoney['mixpeople']){
                        ...
                        $save = [
                            'allmoney' => $addMoney['newmoney']  //当前对应金额加已获得总收益
                        ];

                         $rec = [
                            'money' => $addMoney['money'],
                            'date' => date('Ymd'),
                            'user_id' => $orderInfo['pid']
                        ];
                        DB::name('backmoney_rec') -> where(['user_id' => $orderInfo['pid']]) -> insert($rec); 

                        $is_back = DB::name('backmoney') -> where(['user_id' => $orderInfo['pid']])->find();
                        if($is_back){
                            DB::name('backmoney') -> where(['user_id' => $orderInfo['pid']]) -> update($save); //修改总收益
                        }else{
                            $save['user_id'] = $orderInfo['pid'];
                            DB::name('backmoney') -> where(['user_id' => $orderInfo['pid']]) -> insert($save); //修改总收益
                        }
                        ...
                        DB::name('backmoney_rec') -> where(['user_id' => $orderInfo['pid']]) -> insert($rec); //修改总收益
                ...
                if($orderInfo['memo'] == 2){
                    ...
                    $re = $this->userGetPackage($orderInfo['id']);//购买礼包后增送一个优惠券
    ...
    //***购买合伙人礼包或者会员礼包后, 用户可以随机获取一个消费红包***//
    public function userGetPackage($userid)
    {
        ...
        $data = Db::name('coupon')
            ...
            ->leftJoin('promotion_result pr','pr.promotion_id = p.id')
            ->where('p.etime','>',time())
            ->where('p.is_package','=',1)
            ->where('c.user_id','=',null)
            ->field('pr.params,c.coupon_code')
            ->select()->toArray();
        if(sizeof($data) > 0){
            $sqlData = [
                'user_id'=>$userid,
                'get_date'=>date('Ymd',time())
            ];
            $res = DB::name('coupon') -> where(['coupon_code' => $data[0]['coupon_code']]) -> update($sqlData);
            if ($res) return 'ok'; else return 'fail';
        }else{
            return 'fail';
        }
    }
    ...
    public function allMoney($info,$goods){
        $userUp = DB::name('user') -> where(['id' => $info['pid']]) -> find();//获取邀请人的身份
        //查看当前购买人的上级一共有多少个下级已经购买礼包
        $sonCount = DB::name('user') -> where(['pid' => $info['pid']]) -> count();
        file_put_contents('tttt.txt', $sonCount);
        //获取不同身份对应的上限人数
        if($userUp['grade'] == 2){
        ...
        $compensate = '';
        $getMoney = '';//给上级的返利总额
        $all_compensate =  '';//佣金升级后的 补齐差价额
        foreach ($goods as $k=>$v){
            //循环每个商品都返回上级佣金
            $brokerages = Db::name('goods')->where('id',$v['goods_id'])->find();

            //如果下级人数 <  佣金升级人数, 那么用 佣金升级之前的佣金返利
            if( $sonCount < $peopleMix ){
                switch ($userUp['grade']){
                    case (2):
                        $back = $brokerages['member_before'];
                        break;
                    case (3):
                        $back = $brokerages['partner_before'];
                        break;
                   default:
                       $back = $brokerages['normal_before'];
                       break;
                }

            }else{
                //如果下级人数 > 佣金升级人数, 那么用 佣金升级之后的佣金返利.
                //并且还要补齐差价 例如佣金升级人数是20人 需要用升级后的金额-升级前的金额 * 20份, 补给上级.
                switch ($userUp['grade']){
                    case 2 :
                        $back = $brokerages['member_after'];
                        $compensate = $back - $brokerages['member_before'] > 0 ? $back - $brokerages['member_before'] : 0;
                        $compensate = $compensate*$peopleMix;//每份差价 乘以 升级人数 = 总的补齐的差价
                        break;
                    case 3 :
                        $back = $brokerages['partner_after'];
                        $compensate = $back - $brokerages['partner_before'] > 0 ? $back - $brokerages['partner_before'] : 0;
                        $compensate = $compensate*$peopleMix;//每份差价 乘以 升级人数 = 总的补齐的差价
                        break;
                   default:
                       $back = $brokerages['normal_after'];
                       $compensate = $back - $brokerages['normal_before'] > 0 ? $back - $brokerages['normal_before'] : 0;
                       break;
                }
            }
            $all_compensate+=$compensate;//佣金升级后的 补齐差价额
            $getMoney += $back;//此次购买订单给上级的总返佣金额
        }
        $getMoney = $getMoney + $all_compensate;
        $newMoney = $all['allmoney'] + $getMoney;

        //返回
        $data = [
            'allCount' => $sonCount, //该用户已购买礼包人数
            'money' => $getMoney , //当前返佣总钱数
    ...
    //数据逻辑处理
    public function allMoney_bak($info){
        $userUp = DB::name('user') -> where(['id' => $info['pid']]) -> find();//获取邀请人的身份
        //查看用户一共有多少个下级已经购买礼包

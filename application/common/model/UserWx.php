...
//    }
    ...
    public  function toAdd($data,$pid){
        ...
        if ($info->user_id == 0) {
            ...
            $userModel = new \app\common\model\User();
            $userinfo = $userModel::get($info->user_id);
            //如果是登录过的老用户，查看temp = 0，说明没有上级，需要绑定上级。 如果temp=2 或者 1 说明有上级 不做任何操作
            if( $pid != 0 && $userinfo['temp'] == 0 ){//如果pid！=0 那么就是有分享码进来的，需要绑定上级。
                $rdata = ['pid'=>$pid,'temp'=>1,'btime'=>time()];
                $res = $userModel->save($rdata,['id'=>$info->user_id]);
            }
            //绑定好手机号码了，去登陆,去取user_token
            $userTokenModel = new UserToken();

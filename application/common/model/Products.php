...
class Products extends Common
{
     ...
     */
    public function getProductInfo($id, $isPromotion = 1, $user_id = 0)
    {
        $result  = [
            'status' => false,
            'msg'    => '获取失败',
        ...
        $goods = $goodsModel->where(['id' => $product['goods_id']])->field('name,image_id,bn,marketable,spes_desc,package')->find();//后期调整
        //判断如果没有商品，就返回false
        if (!($goods)) {
            return $result;
        }
        $product['package'] = $goods['package'];

...
//            }
    ...
    public function manageAdd($data)
    {
        ...
        file_put_contents('ooo.txt', $data['pid']);
        $newData['temp'] = ($data['pid'] == 0) ? 0 : 1;//如果是新用户 temp 默认为 1， 既临时下级，购买礼包成功后改为 2，永久下级
        $newData['btime'] = ($data['pid'] == 0) ? '' : time();
    ...
    public function manageEdit($data)
    {
        ...
        $newData['status']   = $data['status'] == null?1:$data['status'];

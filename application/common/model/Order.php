...
class Order extends Common
{
    ...
    protected function getListByWhere($input, $isPage = true)
    {
        ...
        if($manageInfo['username'] != 'admin'){
            $where2 = "find_in_set($manage_id,o.supports)";
        }else{
            $where2 = '';
        }
        //单个订单模糊搜索时
...
//    }
    ...
    protected function orderCount($id = 0, $user_id = false)
    {
        ...
        $where = [];
        if($manageInfo['username'] != 'admin'){
            $where2 = "find_in_set($manage_id,supports)";
        }else{
            $where2 = '';
        }
        //都需要验证的
    ...
    public function toAdd($user_id, $order_type, $cart_ids, $delivery, $memo, $point = 0, $coupon_code = false, $formId = false, $source = 2, $tax = [], $params = [])
    {
        ...
        try {
            ...
            foreach ($items as $k => $v) {
                $sid = Db::name('goods')->find($v['goods_id']);
                $support[$k] = $sid['lower_id'];
                //更改库存
            ...
            $str = implode(',',$support);
            $orid = $items[0]['order_id'];
            Db::name('order')->where('order_id',$orid)->setField('supports',$str);

...
class Coupon extends Common
{
     ...
     * 获取今天以后所有可用的消费红包优惠券
     */
    public function todayPackage($stime)
    {
        $data = $this->alias('c')
            ->join('promotion p','c.promotion_id = p.id')
            ->leftJoin('promotion_result pr','pr.promotion_id = p.id')
     ...
     * @throws \think\exception\DbException
     */
    public function getMyCoupon($user_id, $promotion_id = '', $display = 'no_used')
    {
        $return = [
            'status' => false,
            'msg' => '获取失败',
        ...
        if($return['data']['list'] !== false)
        {
            ...
            if(count($return['data']['list']) > 0)
            {
                ...
                foreach($return['data']['list'] as $k => $v)
                {
                    ...
                    unset($return['data']['list'][$k]['promotion_id']);
                    unset($return['data']['list'][$k]['is_used']);
                    unset($return['data']['list'][$k]['user_id']);
                    unset($return['data']['list'][$k]['used_id']);
                    unset($return['data']['list'][$k]['ctime']);
                    unset($return['data']['list'][$k]['utime']);
                    unset($return['data']['list'][$k]['type']);
                    unset($return['data']['list'][$k]['sort']);
                    unset($return['data']['list'][$k]['exclusive']);
                    unset($return['data']['list'][$k]['auto_receive']);
                    unset($return['data']['list'][$k]['params']);
                    unset($return['data']['list'][$k]['isdel']);
                    ...
                    $money = Db::name('coupon')
                        -> alias('c')
                        -> join('promotion_result pro' , 'c.promotion_id = pro.promotion_id')
                        -> where(['c.coupon_code' => $v['coupon_code']])
                        -> value('params');
                    $money = json_decode($money , true)['money'];
                    $return['data']['list'][$k]['money'] = $money;
                }

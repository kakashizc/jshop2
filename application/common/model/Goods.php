...
//                $list[$key]['label_ids'] = getLabel($value['label_ids']);
            ...
            //在结果集中添加会员价格
            $vprice = Db::name('discount')->select();
            foreach ($vprice as $pk =>$pv){
                if($pv['check'] == 2){ //会员
                    $vdis = $pv['discount'];
                }
                if($pv['check'] == 3){ //会员
            ...
            foreach ($newarr as $ks=>$vs){
                // $newarr[$ks]['vprice'] = $vs['price']*$vs['vdis'];
                // $newarr[$ks]['mprice'] = $vs['price']*$vs['mdis'];
                if( $vs['package'] == 2 || $vs['package'] == 3 ){
                    $newarr[$ks]['vprice'] = $vs['price'];
                    $newarr[$ks]['mprice'] = $vs['price'];
                }
            }
            $result['data']  = $newarr;
...
//            }
            ...
            $list['vprice']   = $list['price']*$list['vdis'];
            $list['mprice']   = $list['price']*$list['mdis'];
            if( $list['package'] == 2 || $list['package'] == 3 ){
                $list['vprice']   = $list['product']['price'];
                $list['mprice']   = $list['product']['price'];
            }
            if ($list['spes_desc']) {
            ...
            $list['album'] = $album;
            //查找轮播视频
            $video = Db::name('goods_video')->where(['goods_id' => $list['id']])->order('id desc')->select()->toArray();
            $goods_video = [];
            if(!empty($video)){
                foreach ($video as $kk=>$vv){
                    $goods_video[$kk] = $vv['video'];
                }
            }
            $list['video'] = $goods_video;
            //取出销量
            $orderItem         = new OrderItems();
            $count             = $orderItem->where(['goods_id' => $gid])->sum('nums');
            //客户为了增加显示的销量, 后台设置了一个虚拟销量, 真实销量 + 虚拟销量 = 小程序前端显示的销量!!!
            $mask_count = Db::name('goods')->where('id',$gid)->value('mask_count');
            $list['buy_count'] = $count+$mask_count;
    ...
    public function getPrice($product, $user_id = '')
    {
        ...
        //获取用户状态: 如果是会员,合伙人等, 那么price 需要乘以 每个商品对应的折扣
        $who = Db::name('user')->find($user_id);
        $type = $who['grade'];
        $dis = $this::get($product['goods_id'])->toArray();
        
        //如果这个商品被选为 红包专区商品了,那么将不会享受折扣
        if ( $dis['is_red'] == 1 ){
            $fprice = $product['price'];
        }else{
            if($type == '2'){ //会员 vdis
                $fprice = $product['price'] * $dis['vdis'];
            }else if($type == '3'){
                $fprice = $product['price'] * $dis['mdis'];
            }else{
                $fprice = $product['price'];
            }
        }

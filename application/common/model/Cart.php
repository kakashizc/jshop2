...
class Cart extends Common
{
    ...
    public function getList($userId, $ids, $type = 1)
    {
        ...
        foreach($list as $k => $v){
            ...
            //如果购物车中的商品包含礼包产品,也就是package=2 或 3 的商品,就把返回的memo 改成 1, 否则memo=0. 前端就方便判断了.
            if (isset($productInfo['data']['package']) && $productInfo['data']['package'] == 2 || $productInfo['data']['package'] == 3){
                $result['memo'] = 2;
            }
            $list[$k]['memo'] = $productInfo['data']['package'];
            if(!$productInfo['status']){
                unset($list[$k]);
                $this::destroy($v['id']);
                continue;
    ...
    public function info($userId, $ids, $order_type = '1', $area_id = 0, $point = 0, $coupon_code = "", $free_freight = false, $delivery_type = 1)
    {
        ...
        if(!$cartList['status']){
            ...
            $result['data']['memo'] = $cartList['memo'];
        }

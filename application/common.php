...
function cmf_api($code,$msg,$data='')
{
    $return = [];
    $return['code'] = $code;
    $return['msg'] = $msg;
...
//上传图片接口
function upload_img($files,$imagesExt=['jpg','png','jpeg','gif'])
{
    // 判断错误号
    if (@$files['error'] == 00) {
        ...
        // 进行文件移动
        $file = $files->move(ROOT_PATH . 'public/upload/video');
        $file =$file->getSaveName();
        return $file;

...
use app\common\model\GoodsVideo as goodsVideoModel;
...
class Goods extends Manage
{
    ...
    public function  add_product_video()
    {
        if (!Request::isAjax()) {
            ...
            return $this->fetch('add_product_video');
        }
    }
    ...
    public function product_video()
    {
        if(Request::isAjax())
        {
            $carouselModel = new goodsVideoModel();
     ...
     * @return mixed
     */
    public function del_product_video()
    {
        $id = input('id');
        $res = Db::name('goods_video')->delete($id);
        if($res){
            $result  = [
                'status' => true,
                'msg'    => '删除成功',
    ...
    private function checkGoodsInfo($isEdit = false)
    {
        ...
        $data['goods']['package']       = input('post.goods.package','1');
        $data['goods']['vdis']          = input('post.goods.vdis','1');
        $data['goods']['mdis']          = input('post.goods.mdis','1');
        $data['goods']['is_red']        = input('post.goods.is_red',0);
        $data['goods']['normal_before']  = input('post.goods.normal_before');
        $data['goods']['member_before']  = input('post.goods.member_before');
        $data['goods']['partner_before']  = input('post.goods.partner_before');
        $data['goods']['normal_after']  = input('post.goods.normal_after');
        $data['goods']['member_after']  = input('post.goods.member_after');
        $data['goods']['partner_after']  = input('post.goods.partner_after');
        $data['goods']['lower_id']  = input('post.goods.lower_id');
        $data['goods']['mask_count']  = input('post.goods.mask_count');
    ...
    public function edit()
    {
        ...
        //$manager = Db::name('manage')->where('username','neq','admin')->select();
        $manager = Db::name('manage')
            ->alias('manage')
            ->join('manage_role_rel','manage.id=manage_role_rel.manage_id')
            ->join('manage_role','manage_role_rel.role_id = manage_role.id')
            ->where('manage.username','neq','admin')
            ->field('manage.id,manage_role.name as username')
            ->select();

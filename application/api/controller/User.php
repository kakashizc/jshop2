...
class User extends Api
{
    ...
    public function wxappLogin2()
    {
        ...
        $param = input();
        $ar = json_encode($param);
        file_put_contents('52222.txt',$ar);
        ...
        if(input('?param.pid')){
            file_put_contents('344444.txt', input('param.pid'));
        }
        //如果新用户不需要手机号码登陆，但是有推荐人的话，校验推荐人信息
        if (input('?param.pid')) {
            file_put_contents('4.txt',input('?param.invitecode'));
            $userModel = new \app\common\model\User();
            $pid   = $userModel->getUserIdByShareCode(input('param.pid'));
    ...
    /**
     * 此处是老用户，没清除缓存，直接扫上级二维码了，重新绑定为二维码的下级（因为有缓存，不走授权那个登陆绑定上下级判断）
     * 所以这里走一个单独的接口去绑定上下级关系
     */
    public function bind()
    {
        ...
        if (!input("?param.invitecode") ||!input("?param.userToken")  ) {
            $result['msg'] = '参数缺失';
            return $result;
        }
        $token = input("param.userToken");
        $userModel = new \app\common\model\User();
        $pid   = $userModel->getUserIdByShareCode(input('param.invitecode'));
        $uid = Db::name('user_token')->where('token',$token)->find();
        //查看当前用户的temp是否为0，就是临时下级被释放了
        $is_temp = Db::name('user')->where('id',$uid['user_id'])->find();
        if($is_temp['temp'] == 1 || $is_temp['temp'] == 2 ){
            $result['msg'] = '还是下级，未被释放';
            return $result;
        }
        //绑定新上级
        $data['temp'] = 1;
        $data['pid'] = $pid;
        $data['btime'] = time();
        $res = Db::name('user')->where('id',$uid['user_id'])->update($data);
        if($res){
            $result['msg'] = '绑定新上级成功';
            return $result;
        }else{
...
//    }
    ...
    public function profit(){
        ...
        foreach($userList as $k => $v){
            ...
            $userInfo = DB::name('user')
                ...
                -> where(['u.id' => $v['user_id'] , 'o.order_id' => $v['order'] , 'o.pay_status' => 2])
                -> find();
            $userList[$k]['username'] = $userInfo['nickname']?$userInfo['nickname']:'匿名';
            $userList[$k]['headimg'] = $userInfo['avatar']?$userInfo['avatar']:' ';
            $userList[$k]['expenses'] = $userInfo['order_amount']?$userInfo['order_amount']:0;
        }
        $result = [
            'code' => 0,
            'data' => $userList,
    ...
    public function team(){
        ...
        $user_id = DB::name('user_token') -> where(['token' => $usertoken]) -> value('user_id');
        //获取所有下线的信息
        $allDown = DB::name('user') -> where(['pid' => $user_id]) -> column('id , avatar , nickname , grade');
        $allorder = [];
        foreach($allDown as $k => $v){
            //根据下线查出所有order中的金额总和
            $allDown[$k]['allmoney'] = json_decode(DB::name('order') -> where(['user_id' => $v['id'] , 'pay_status' => 2]) -> sum('order_amount') , true);
        }
        $result = [
            'status' => true,
            'data' => $allDown,
